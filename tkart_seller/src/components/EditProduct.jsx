...
const EditProduct = () => {
  ...
  const isMediaComplete = previews[0] !== null;
    ...
    <div className="min-h-screen bg-gray-50 font-josefin pb-12">
      ...
      <div className="relative h-32 md:h-48 bg-gradient-to-br from-hotpink-400 via-hotpink-500 to-hotpink-600 w-full overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-start pt-6 md:pt-10 items-center text-white px-4">
          <h1 className="text-2xl md:text-3xl font-normal text-white tracking-tight drop-shadow-sm mb-1 md:mb-2">Edit Product</h1>
          <p className="text-sm md:text-lg font-normal opacity-90 text-center max-w-2xl">Update details, pricing, and media for {form.name}</p>
        </div>
      </div>
              ...
              {activeTab === 'pricing' && (
                ...
                >
                  <div className="border-b border-gray-100 pb-4 mb-2">
                    ...
                    <p className="text-gray-500 text-sm mt-1">Update your price and stock availability.</p>
                  </div>
                  ...
                  <div className="p-6 border-2 border-gray-100 bg-gray-50/50 rounded-2xl flex items-center justify-between hover:border-hotpink-200 transition-colors">
                    <div>
                      <h4 className="font-normal text-gray-800 text-base">Product Status</h4>
                      <p className="text-sm text-gray-500 font-normal mt-1">Toggle visibility on the storefront</p>
                    </div>
                    <label className="flex items-center cursor-pointer">
                      <div className="relative">
                        <input type="checkbox" className="sr-only" checked={form.in_stock} onChange={() => setForm({...form, in_stock: !form.in_stock})} />
                        <div className={`block w-14 h-8 rounded-full transition-colors ${form.in_stock ? 'bg-hotpink-500' : 'bg-gray-300'}`}></div>
                        <div className={`dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition-transform ${form.in_stock ? 'transform translate-x-6' : ''}`}></div>
                      </div>
                    </label>
                  </div>
                </motion.div>
                ...
                >
                  ...
                  <div className="flex flex-col gap-6">
                    ...
                    <label className="group flex flex-col items-center justify-center h-56 border-2 border-dashed border-hotpink-300 bg-hotpink-50/30 rounded-3xl cursor-pointer hover:bg-hotpink-50 hover:border-hotpink-500 transition-all duration-300 hover:shadow-lg relative overflow-hidden">
                      <input type="file" accept="image/*" onChange={(e) => handleImageChange(0, e.target.files[0])} className="hidden" />
                      {previews[0] ? (
                        <img src={previews[0]} alt="cover" className="w-full h-full object-cover rounded-2xl group-hover:scale-105 transition-transform duration-700" />
                      ) : (
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      {[1, 2, 3].map((i) => (
                        <label key={i} className="group flex flex-col items-center justify-center h-32 border-2 border-dashed border-gray-200 bg-gray-50/50 rounded-2xl cursor-pointer hover:bg-hotpink-50/50 hover:border-hotpink-400 transition-all duration-300 relative overflow-hidden">
                          ...
                          {previews[i] ? (
                            <img src={previews[i]} alt={`gallery-${i}`} className="w-full h-full object-cover rounded-xl group-hover:scale-105 transition-transform duration-500" />
                          ) : (
            ...
            >
              <div className="relative">
                ...
                <div className="absolute inset-0 flex items-center justify-center text-hotpink-500">
                  <FiSave className="text-2xl animate-pulse" />
                </div>
              </div>
              <div className="text-center">
                <h3 className="text-2xl font-normal text-gray-800">Saving Changes...</h3>
                <p className="text-sm text-gray-500 mt-1 font-normal">Updating your product for the storefront.</p>
              </div>
            </motion.div>
          </motion.div>
        )}
