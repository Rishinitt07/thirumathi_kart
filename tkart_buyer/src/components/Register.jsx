...
const Register = () => {
  ...
  const handleSendOtp = () => {
    ...
    axios.post("http://localhost:8081/send-otp", { email: form.email, mobile: form.mobile })
      .then((res) => {
        setOtpSent(true);
        toast.success(
  ...
  const handleVerifyOtp = () => {
    if (!otp) return;
    axios.post("http://localhost:8081/verify-otp", { email: form.email, mobile: form.mobile, otp: otp })
      .then(() => {
        setOtpVerified(true);
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post("http://localhost:8081/info", form)
      .then(() => {
