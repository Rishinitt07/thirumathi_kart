...
const TRACKING_STEPS = [
  ...
  { id: 'shipped', label: 'Order shipped', statusMatch: [ORDER_STATUS.SHIPPED, ORDER_STATUS.OUT_FOR_DELIVERY, ORDER_STATUS.NEAR_DOORSTEP, ORDER_STATUS.DELIVERED] },
  { id: 'out_delivery', label: 'Out for delivery', statusMatch: [ORDER_STATUS.OUT_FOR_DELIVERY, ORDER_STATUS.NEAR_DOORSTEP, ORDER_STATUS.DELIVERED] },
  { id: 'doorstep', label: 'Near your doorstep', statusMatch: [ORDER_STATUS.NEAR_DOORSTEP, ORDER_STATUS.DELIVERED] },
];
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wider border shadow-sm ${color}`}>
      {icon}
      {status}
    </span>
  );
...
const OrderCard = React.memo(
  ({ order, onCancelClick }) => {
    ...
    const grandTotal = order.total || order.items.reduce((sum, item) => sum + item.price * (item.quantity || item.qty), 0);
        ...
        <div className="p-4 sm:p-6 border-b border-gray-100 bg-gray-50/30 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex flex-row items-center justify-between w-full sm:w-auto gap-4">
            <div className="text-left sm:text-right">
              <span className="block text-xs text-gray-500 font-medium uppercase tracking-wider mb-0.5">Order Total</span>
          ...
          {order.items.map((item, idx) => (
            <div key={idx} className="flex flex-col sm:flex-row gap-4 sm:gap-6 border-b border-gray-100 last:border-0 pb-6 last:pb-0">
              ...
              <div className="flex-1 flex flex-col justify-between">
                <div>
                  <Link to={`/product/${item.id}`} className="font-semibold text-lg text-gray-900 hover:text-hotpink-600 transition-colors block mb-1">
                    {item.name}
                  </Link>
                  <p className="text-xs text-green-600 font-medium tracking-wide uppercase mb-1">In stock</p>
                  <div className="flex items-center text-sm text-gray-500 mt-2">
                    <span className="bg-gray-100 px-2.5 py-1 rounded-md font-medium text-gray-700">Qty: {item.quantity || item.qty}</span>
                  </div>
                </div>
              </div>
              <div className="text-left sm:text-right mt-2 sm:mt-0">
                <p className="text-2xl font-normal text-gray-900">₹{(item.price * (item.quantity || item.qty)).toLocaleString()}</p>
          ...
          <div className="flex flex-wrap items-center gap-4 mt-2 pt-4 border-t border-gray-100">
            {canCancel && (
              <button
                ...
                className="text-sm font-semibold text-gray-500 hover:text-red-600 transition-colors flex items-center gap-1.5 px-4 py-2.5 rounded-xl border border-gray-200 hover:border-red-200 hover:bg-red-50 bg-white shadow-sm"
              >
                <FiXCircle size={16} />
                Cancel Order
              </button>
            ...
            <button 
              ...
              className="text-sm font-semibold text-hotpink-600 hover:text-hotpink-700 transition-colors flex items-center gap-1.5 px-4 py-2.5 rounded-xl border border-hotpink-200 bg-hotpink-50 hover:bg-hotpink-100 shadow-sm"
            >
              <FiTruck size={16} />
          ...
          {trackingVisible && (
            ...
            >
              <div className="p-6 sm:p-8 max-w-2xl mx-auto">
                <h4 className="font-bold text-gray-900 mb-6 text-lg">Tracking Progress</h4>
                
                {isCancelled ? (
                  <div className="bg-red-50 p-6 rounded-2xl border border-red-100 text-center">
                    <FiXCircle className="mx-auto text-red-500 mb-3" size={40} />
                    <h5 className="text-red-700 font-bold text-lg">Order Cancelled</h5>
                        ...
                        return (
                          <div key={step.id} className="flex items-center">
                            ...
                            <div className="ml-6 sm:ml-8">
                              <p className={`font-semibold text-base ${isActive ? 'text-gray-900' : 'text-gray-400'}`}>
                                {step.label}
                              </p>
                              {isCurrent && (
                                <p className="text-xs text-hotpink-600 font-medium mt-0.5">Current Status</p>
                              )}
                            </div>
                          </div>
                        )
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="bg-gray-50 rounded-2xl p-6 border border-gray-100 shadow-sm">
                    ...
                    <div className="text-gray-600 text-sm space-y-1.5">
                      <p className="font-semibold text-gray-900 text-base mb-1">{order.username}</p>
                      <p className="leading-relaxed">{order.address}</p>
                      <p>{order.city}, {order.state} - {order.pincode}</p>
                      <p className="mt-3 pt-3 border-t border-gray-200 font-medium flex items-center gap-2">
                  ...
                  <div className="bg-gray-50 rounded-2xl p-6 border border-gray-100 shadow-sm">
                    ...
                    <div className="text-gray-600 text-sm space-y-3">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="font-semibold text-gray-800 uppercase bg-white border border-gray-200 px-3 py-1 rounded-md shadow-sm">{order.payment_method || 'COD'}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-gray-500">Items Subtotal</span>
                        <span className="font-medium text-gray-800">₹{grandTotal.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-gray-500">Delivery Fee</span>
                        <span className="font-medium text-green-600 bg-green-50 px-2 py-0.5 rounded text-xs">FREE</span>
                      </div>
                      <div className="border-t border-gray-200 pt-3 mt-1 flex justify-between font-bold text-lg text-gray-900">
      ...
      <div className="min-h-screen py-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto relative z-10">
        ...
        {loading ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-red-50 border border-red-100 p-8 rounded-3xl text-center shadow-sm my-12">
            ...
            <Link to="/login" className="mt-6 inline-block font-bold text-white bg-red-600 px-6 py-3 rounded-xl hover:bg-red-700 transition-colors shadow-sm">
              Login to continue
            </Link>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-24 bg-white rounded-[2rem] shadow-sm border border-gray-100">
            ...
            <div className="mt-10">
              <Link
                to="/categories"
                className="inline-flex items-center px-8 py-4 border border-transparent rounded-2xl shadow-md text-lg font-bold text-white bg-hotpink-600 hover:bg-hotpink-700 hover:shadow-lg transition-all transform hover:-translate-y-1"
              >
                Browse Products
              </Link>
            </div>
          ...
          <>
            ...
            >
              <div className="p-8">
                ...
                <h3 className="text-2xl font-bold text-center text-gray-900 mb-2">Cancel Order</h3>
                ...
                <div className="space-y-4">
                  ...
                  <select 
                    ...
                    className="w-full bg-gray-50 border border-gray-200 text-gray-900 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-hotpink-500 focus:border-transparent transition-all font-medium appearance-none"
                  >
                ...
                <div className="mt-10 flex gap-4">
                  ...
                  <button
                    ...
                    className="flex-1 py-3.5 px-4 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 transition-colors shadow-md hover:shadow-lg"
                  >
                    Confirm Cancel
                  </button>
                </div>
